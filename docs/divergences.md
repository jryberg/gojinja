...
| Iteration order of dicts passed as a raw Go `map[string]any` | Python dicts preserve insertion order; `json.loads` honors source order. | All user-supplied values are normalised at the `RenderContext` boundary: every `map[string]any` / `map[any]any` — top-level **and** nested at every depth — becomes a `*runtime.OrderedDict` with **lex-sorted** keys. `*OrderedDict` inputs (and `JSONVars` output) keep their original insertion order. The same input therefore always renders to the same bytes, but a Go map literal cannot produce Python's "as you wrote it" order — Go's runtime does not carry it. Pass `JSONVars` output or build an `OrderedDict` directly when Python-matching source order is required. | Go maps are randomised; pure lex sort is the closest deterministic substitute. |
